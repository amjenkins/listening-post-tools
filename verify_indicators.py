...
def fetch_twelvedata_quote(symbol):
    """Returns (close_float, as_of_iso8601_string) from Twelve Data's /quote
    endpoint, or raises with a short, specific reason. Requires the
    TWELVE_DATA_API_KEY repository secret to be set (see .github/workflows/
    verify.yml) — confirmed live 2026-09-13 (via diagnose_twelvedata.py)
    that Max's free-tier key covers gold (as XAU/USD) and both FX pairs,
    but NOT stock indices or true commodities — see the ROWS config above
    for exactly which rows use this fallback."""
