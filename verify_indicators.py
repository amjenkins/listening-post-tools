...
fails, gold / EUR/USD / USD/JPY fall back to Twelve Data (Max's free-tier
key), confirmed to actually serve those three; S&P 500, Brent crude, and
DXY have no working second source at all right now — Twelve Data's free
plan doesn't cover stock indices or true commodities, and no other free,
same-convention source has been identified yet. Those three rows report
"SINGLE-SOURCE ONLY" honestly rather than silently passing.
...
ROWS = [
    ...
    },
    {
        "key": "gold",
        "label": "Gold (spot)",
        # Changed 2026-09-13: was "GC%3DF" (COMEX gold FUTURES). The row has
        # always been labeled "Gold (spot)" but was quietly sourcing a
        # futures price the whole time — a real, silent mismatch that only
        # surfaced once a genuine spot-gold second source (Twelve Data's
        # XAU/USD) was added and showed a consistent ~1.4% gap against it.
        # Futures and spot are different instruments with a real, structural
        # basis between them — no amount of timing-alignment fixes that.
        # XAUUSD=X is Yahoo's actual spot-gold symbol, same naming
        # convention as the EUR/USD and USD/JPY rows below.
        "yahoo_symbol": "XAUUSD%3DX",
        "stooq_symbol": "xauusd",
        "decimals": 2,
        # Twelve Data fallback — confirmed live 2026-09-13 that Max's free
        # plan DOES serve gold via this forex-style symbol, even though
        # Twelve Data's own pricing page lists "commodities" as paid-only;
        # apparently precious-metals spot pairs are billed as forex, not
        # commodities. See the module docstring for the full story.
